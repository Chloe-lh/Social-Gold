...
class RemoteCommentsTests(AuthenticatedAPITestCase):
    def setUp(self):
        super.setUp()
        Node.objects.create(id="http://nodebbb/", auth_user="remoteuser", auth_pass="remotepass")
        ...
        def test_remote_comments(self, mock_get):
            ...
            enc = quote(remote_entry, safe='')
            res = self.client.get(f"/api/entries/{enc}/comments/")
            self.assertEqual(res.status_code, 200)

            # assert your code called the expected remote URL
